...
def register(subparsers: argparse._SubParsersAction) -> None:
    ...
    import_statement_p = finance_sub.add_parser(
        "import-statement", help="Import spend transactions from a bank/card statement image or PDF")
    import_statement_p.add_argument("path")
    import_statement_p.add_argument("--mime-type", default=None,
                                     help="Override the guessed mime type, e.g. application/pdf")
    import_statement_p.set_defaults(func=_cmd_import_statement)
...
def _cmd_import_statement(args: argparse.Namespace, kanna: Kanna) -> int:
    svc = _service(kanna)
    try:
        with open(args.path, "rb") as fh:
            file_bytes = fh.read()
    ...
    provider = AnthropicDocumentProvider(model=kanna.settings.llm_model,
                                          max_tokens=kanna.settings.llm_max_tokens)
    result = import_statement(file_bytes, mime_type=mime_type, tx_repo=svc.transactions,
                               cat_repo=svc.categories, provider=provider,
                               default_currency=svc.default_currency)
    ...
    if document_level_error and result.created == 0 and result.skipped_duplicates == 0:
        print(f"error: {document_level_error.error}", file=sys.stderr)
        return 1

    print(f"Imported {result.created} transaction(s), skipped {result.skipped_duplicates} duplicate(s), "
          f"{len(result.errors)} row(s) not imported.")
    for err in result.errors:
        print(f"  row {err.row_number}: {err.error}", file=sys.stderr)
    return 0
